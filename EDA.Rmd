...
Time_series_income("une_life",merged_data)
```
...
merged_data_temp =
merged_data %>% 
  select(year, une_life, health_exp) %>% 
  filter(year > 2012)
...
for (i in 2013:2016){
    myplots[[i-2012]] =
      merged_data_temp %>% 
      filter(year == i) %>% 
      ggplot(aes(x = health_exp, y = une_life))+geom_point()+geom_smooth(method = 'lm', se = TRUE, color = 'red')

...
```{r}
women_in_parliaments = read_csv("data/around_2019_women_percent_in_national_parliaments.csv", skip = 5) %>% 
  janitor::clean_names() %>% 
  mutate("country"=x2,
         "women_percent" = percent_w_6) %>% 
  select(country, women_percent)

merged_data = 
  merged_data_h %>% 
  left_join(women_in_parliaments, by = c("country")) %>% 
  select(country_code:une_school,PM_value,income_group,health_exp, women_percent,`Developed / Developing Countries`)

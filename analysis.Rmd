...
data %>% 
  janitor::clean_names() %>% 
  group_by(year) %>% 
  summarise(avg_by_year = mean(une_life,na.rm = T)) %>% 
  arrange(avg_by_year) %>% 
...
data %>% 
  janitor::clean_names() %>% 
  group_by(year)  %>% 
  ggplot(aes(x = factor(year),y = une_life,fill = year)) +
  geom_boxplot() +
...
\ \par
## 4. prop.test : Compare the Proportion of Life Expectancy Over 70 Between Western Pacific and South-East Asia
...
\ \par
## 8. t.test : Comapre Mean Life Expectancy Between Developed and Developing Countries

...
Visualization via scatter plot is a way to further discover the pairwise relation between two chosen variable.  
Since we choose Life expectancy as our response variable, we can plot the scatter plot between life expectancy and other chosen predictors to see the correlation between variables. 
...
plot_row1 <- plot_grid(myplots[[1]], myplots[[2]],labels = c(2013,2014))
plot_row2 <- plot_grid(myplots[[3]], myplots[[4]],labels = c(2015,2016))
# title
title <- ggdraw() + 
  draw_label(
...
merged_data_temp =
merged_data %>% 
  select(year, life_expect, measles) %>% 
  filter(year > 2012)
...
for (i in 2013:2016){
    myplots[[i-2012]] =
      merged_data_temp %>% 
      filter(year == i) %>% 
      ggplot(aes(x = measles, y = life_expect))+geom_point()+geom_smooth(method = 'lm', se = TRUE, color = 'red')
...
merged_data_temp =
merged_data %>% 
  select(year, life_expect, health_exp) %>% 
  filter(year > 2012)
...
for (i in 2013:2016){
    myplots[[i-2012]] =
      merged_data_temp %>% 
      filter(year == i) %>% 
      ggplot(aes(x = health_exp, y = life_expect))+geom_point()+geom_smooth(method = 'lm', se = TRUE, color = 'red')
...
title <- ggdraw() + 
  draw_label(
    "Scatter Plot for Life Expectancy and health_exp",
    fontface = 'bold',
    x = 0,
    hjust = 0
  ) +

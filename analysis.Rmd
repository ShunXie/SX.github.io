...
\ \par
## 3. t.test : Compare Mean Life Expectancy Between Americas and Europe
...
\ \par
## 4. t.test : Compare Mean Life Expectancy Between Western Pacific and Eastern Mediterranean

#### extract life expectancy in Western Pacific and Eastern Mediterranean
```{r}
Western_Pacific <- data %>% 
  janitor::clean_names() %>% 
  filter(region == "Western Pacific") %>% 
  pull(life_expect)

Eastern_Mediterranean <- data %>% 
  janitor::clean_names() %>% 
...
#### test equal variance
```{r}
var.test(Western_Pacific,Eastern_Mediterranean,alternative = "two.sided",conf.level = 0.95) %>% 
  broom::tidy() %>% 
  knitr::kable()
...
```{r}
t.test(Western_Pacific,Eastern_Mediterranean,alternative = "two.sided",conf.level = 0.95,paired = F,var.equal = TRUE ) %>% 
  broom::tidy() %>% 
  knitr::kable()

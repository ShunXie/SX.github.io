...
# Additional Steps on Model 1--Transformation on Response Variable

First, we want to find whether response variable une_life needs some transformation.

```{r,message=FALSE, warning=FALSE}
inverseResponsePlot(m1, key = TRUE)
```

From the aboveplot, the best $\lambda$ for model 1 is 2.17794, which is different from 1. Therefore, we will apply $\lambda$ equal to 2.17794 to une_life.
...
The transformed model 1's R-adjusted score reaches `r summary(m1_tr)$adj.r.squared`. The adjusted R square increases ~0.01 after transformation.

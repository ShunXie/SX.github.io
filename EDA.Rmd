...
merged_data_temp =
merged_data %>% 
  select(year, une_life, adult_mortality) %>% 
  filter(year > 2012)
...
for (i in 2013:2016){
    myplots[[i-2012]] =
      merged_data_temp %>% 
      filter(year == i) %>% 
      ggplot(aes(x = adult_mortality, y = une_life))+geom_point()+geom_smooth(method = 'lm', se = TRUE, color = 'red')+
...
title <- ggdraw() + 
  draw_label(
    "Scatter Plot for Life Expectancy and adult mortality",
    fontface = 'bold',
    x = 0,
    hjust = 0
  ) +
